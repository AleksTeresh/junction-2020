...
export function getSuggestionMesssages(statistic, goals, goalResults) {
  ...
  const failureMessages = Object.entries(goalResults).map(([categoryName, ratio]) => {
    ...
    const limit = goal.updates[goal.updates.length - 1].percentage * statistic.income
    if (ratio > 1.6) {
      allSuccess = false
      return `You've seriously missed your goal on ${categories[categoryName].humanString} last month. Indeed, you've went ${(ratio - 1) * limit} $ over your limit! YOu really need to get your act together next month`
    } else if (ratio > 1.0) {
      allSuccess = false
      return `You've slightly missed your goal on ${categories[categoryName].humanString} last months. Just saving up ${(ratio - 1) * limit} $ and you would achieve your goal!`
    }
